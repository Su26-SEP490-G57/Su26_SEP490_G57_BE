  ...
  describe('POST /symptom-surveys', () => {
    ...
    describe('GIVEN an assessmentType of SCHEDULED', () => {
      ...
      describe('AND an open, pending task matches the case/POD/slot', () => {
        ...
        // submitSurvey persists assessment detail rows for every submission
        // (SCHEDULED and TRIGGERED alike).
        it('THEN should persist assessment detail rows for the SCHEDULED submission', async () => {
          const now = new Date();
          await dataSource.getRepository(AssessmentTask).save({
            caseId: 'CASE-001',
            podContext: 2,
